...
const NewVehicle = async (req, res) => {
  ...
  try{ 
    ...
    const ValidaUser = await pool.db('Unipark').collection('vehiculos').findOne({user: datos.correo, rol: Role });
    if(!ValidaUser){

      // Registro el user
      const registroUser =  await pool.db('Unipark').collection('vehiculos').insertOne({user: datos.correo, password: PasswordEncrypt, rol: Role });
      ...
      if (registroUser.acknowledged) {
        ...
        const DatosUser = await pool.db('Unipark').collection('vehiculos').findOne({user: datos.correo, password: PasswordEncrypt });
        if (DatosUser) {
